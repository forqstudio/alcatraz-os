...
let
  ...
  alcatraz-install = pkgs.writeShellScriptBin "alcatraz-install" ''
    ...
    disko --mode destroy,format,mount --disk main "$DISK" /etc/alcatraz/hosts/alcatraz-iso/disko-config.nix
    ...
    cat > /mnt/etc/nixos/hosts/alcatraz/bootloader.nix << 'BOOTLOADER'
